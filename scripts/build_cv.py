...
CANVAS_CONFIGS: dict[str, dict[str, Any]] = {
    DEFAULT_CANVAS_NAME: {
        "label": "Modern CV",
        "directory": CANVASES_DIR / DEFAULT_CANVAS_NAME,
        "tex_root": "main.tex",
        "defaults": MODERN_CV_TEMPLATE,
    },
}
...
def available_canvas_names() -> list[str]:
    return sorted(CANVAS_CONFIGS)


def available_canvases() -> list[dict[str, str]]:
    return [
        {"name": name, "label": str(config.get("label", name))}
        for name, config in sorted(CANVAS_CONFIGS.items())
    ]


def normalize_canvas_name(canvas_name: str | None) -> str:
    name = (canvas_name or DEFAULT_CANVAS_NAME).strip() or DEFAULT_CANVAS_NAME
    if name not in CANVAS_CONFIGS:
        supported = ", ".join(available_canvas_names())
        raise ValueError(f"Canvas inconnu : {name}. Canvas disponibles : {supported}")
    return name


def canvas_config(canvas_name: str | None = None) -> dict[str, Any]:
    return CANVAS_CONFIGS[normalize_canvas_name(canvas_name)]


def canvas_source_dir(canvas_name: str | None = None) -> Path:
    directory = canvas_config(canvas_name)["directory"]
    if not isinstance(directory, Path):
        raise TypeError("Canvas directory must be a pathlib.Path")
    return directory


def canvas_tex_root(canvas_name: str | None = None) -> str:
    tex_root = canvas_config(canvas_name).get("tex_root", "main.tex")
    return str(tex_root)


def default_cv_data(canvas_name: str | None = DEFAULT_CANVAS_NAME) -> dict[str, Any]:
    defaults = canvas_config(canvas_name)["defaults"]
    if not isinstance(defaults, dict):
        raise TypeError("Canvas defaults must be a dictionary")
...
def build_document(
    data: dict[str, Any],
    source_file: str | Path = "database",
    canvas_name: str | None = DEFAULT_CANVAS_NAME,
) -> str:
    canvas_name = normalize_canvas_name(canvas_name)
    ...
    out: list[str] = [
        ...
        f"% Generated by build_cv.py from {source_file}",
        f"% Canvas: {canvas_name}",
        f"% !TeX root = {canvas_tex_root(canvas_name)}",
        "",
...
def copy_build_inputs(work_dir: Path, canvas_name: str | None = DEFAULT_CANVAS_NAME) -> None:
    copy_source_files(canvas_source_dir(canvas_name), work_dir)
...
def compile_latex_pdf(
    ...
    tex_root = canvas_tex_root(canvas_name)
    run_command(
    ...
    if not pdf_path.exists():
        raise RuntimeError(f"LaTeX compilation did not produce {pdf_path.name}.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--canvas",
        default=DEFAULT_CANVAS_NAME,
        choices=available_canvas_names(),
        help="Canvas/template to use",
    )
    parser.add_argument(
        "--compile-pdf",
        action="store_true",
...
def main() -> int:
    ...
    canvas_name = normalize_canvas_name(args.canvas)
    ...
    if args.input:
        ...
        data = default_cv_data(canvas_name)
        source_file = f"built-in {canvas_name} template"

    if not isinstance(data, dict):
        raise TypeError("Root YAML node must be an object")

    generated = build_document(data, source_file=source_file, canvas_name=canvas_name)
